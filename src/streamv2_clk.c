...
void streamv2_usage(int argc, char *argv[], char *usage)
{
...
int main(int ac, char **av)
{
  ...
  for (j = 0; j < NUMSIZES; ++j)
  {
    ...
    expt = log((double)(NMIN)) +
           ((double)j / (double)(NUMSIZES - 1)) * (log((double)NMAX) - log((double)NMIN));
...
double compute_tdelta(timebasestruct finish, timebasestruct start)
{
  ...
  if (nsecs < 0)
  { /* check for carry and fix if needed */
    secs--;
    nsecs += 1000000000;
  }
